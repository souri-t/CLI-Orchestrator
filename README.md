...
**1 Issue = 1 PR = 1 完結した作業** を前提として設計されています。

#### ✅ 正しい使い方

```
Issue #7 に ai-task → Draft PR #8 が作成される
     ↓
PR #8 を main にマージ
     ↓
新しい Issue #9 を作成して ai-task → 続きの作業を依頼
```
...
Issue #7 に ai-task → Draft PR #8 が作成される
     ↓
PR #8 が未マージのまま、Issue #7 に追加要望をコメント → ai-task を再付与
     ↓
サンドボックスは常に main ブランチをクローンするため、
PR #8 の変更がコード上に存在せず、AI が作業できずに失敗する
```

> **制約の理由**: サンドボックスは常にデフォルトブランチ（`main`）を起点にコードをチェックアウトします。未マージの PR の変更はサンドボックス内のコードに反映されないため、その変更を前提とした作業は実行できません。
>
> PR をマージしてからコードベースを最新に保った状態で次の Issue を起票してください。
